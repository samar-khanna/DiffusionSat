...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def check_image(self, image, prompt, prompt_embeds):
        ...
        image_is_list_pil_list = isinstance(image, list) and isinstance(image[0], list) and isinstance(image[0][0], PIL.Image.Image)
        image_is_list_tensor_list = isinstance(image, list) and isinstance(image[0], list) and isinstance(image[0][0], torch.Tensor)

        if not image_is_pil and not image_is_tensor and not image_is_pil_list and not image_is_tensor_list and not image_is_list_pil_list and not image_is_list_tensor_list:
            raise TypeError(
        ...
        elif image_is_pil_list or image_is_list_pil_list:
            image_batch_size = len(image)
        elif image_is_tensor_list or image_is_list_tensor_list:
            image_batch_size = len(image)
    ...
    def prepare_image(
        ...
        if not isinstance(image, torch.Tensor):
            ...
            elif isinstance(image[0], list) and (isinstance(image[0][0], PIL.Image.Image) or isinstance(image[0][0], torch.Tensor)):
                images = []
                for img_list in image:
                    image_stack = self.prepare_image(img_list, width, height, batch_size, 1,
                                                     device, dtype, False, guess_mode)
                    image_stack = torch.cat([t for t in image_stack], dim=0).unsqueeze(0)  # (1, len(stack)*c, h, w)
                    images.append(image_stack)

                image = torch.cat(images, dim=0)  # (1, len(stack)*c, h, w)
